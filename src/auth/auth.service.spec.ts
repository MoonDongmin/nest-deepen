...
describe('AuthService', () => {
  ...
  beforeEach(async () => {
    ...
    cachManager = module.get<Cache>(CACHE_MANAGER);
  ...
  describe('tokenBlock', () => {
    it('should  block a token', async () => {
      ...
      expect(cachManager.set).toHaveBeenCalledWith(
        `BLOCK_TOKEN_${token}`,
        payload,
        expect.any(Number),
      );
  ...
  describe('parseBearerToken', () => {
    ...
    it('should throw a BadRequestException for token out starting with Bearer', () => {
      const rawToken = 'Basic a';
      expect(authService.parseBearerToken(rawToken, false)).rejects.toThrow(
        BadRequestException,
      );
    ...
    it('should throw a BadRequestException if payload. type is refresh but isRefreshToken parameter ', () => {
  ...
  describe('authenticate', () => {
    it('should authenticate a user with correct credentials', async () => {
      ...
      expect(bcrypt.compare).toHaveBeenCalledWith(password, 'asdfasdf');
      expect(result).toEqual(user);
    });
    ...
    it('should  throw an error for incorrect password', async () => {
      ...
      await expect(
        authService.authenticate('test@test.com', 'asdf'),
      ).rejects.toThrow(BadRequestException);
    });
  });
